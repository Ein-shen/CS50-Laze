...
const ShareStudy = () => {
  ...
  useEffect(() => {
    ...
    // No login, no saving — just show the result screen locally
    useEffect(() => {
        if (finished) {
            navigate(`/sharetotal/${deckId}`, {
        ...
        <div className="w-screen h-screen bg-gray-300 flex flex-col items-center gap-4 pt-10 ">
            ...
            <div className="rounded-md relative flex items-center justify-center border-black border-2 gap-10 w-[55%] h-auto px-6">
                <div className="pb-10 pt-10 flex flex-row">
                    <button type="button" className="absolute left-5 top-1/2 -translate-y-1/2 flex items-center justify-center" onClick={() => navigate(`/stalk/${ownerUsername}`)}>
                ...
                <div className="absolute right-5 flex flex-col gap-1">
                    <p className="font-bold text-md">Card: {index + 1} / {cards.length}</p>
                    <p className="font-bold text-md">Score: {score}</p>
                </div>
            </div>

    ...
    try:
        ...
        if bay:
            ...
            if buntag_hapon == "PM":
                ...
                for i in time_AM:
                    if first_G_orasan == i[0]:
                        ...
                        # print(converted_slyde)
                                                        # 10:
                    elif hapon_buntag == "PM":                            #  10:11 AM to 8:22 PM
                            for i in time_PM:                              # 9 AM to 5 AM
                                if fourth_G_orasan == i[0]:                    # 9 PM to 5 PM
                                    sinaw = i[1]                                # 9 AM to 5 PM
                                    converted_sinaw = int(sinaw)                  # 9 PM to 5 AM
                                                                            # 10 AM to 8:50 PM
            ...
            if  new_second_G_minuto and new_fifth_G_minuto:
                 return(f"{converted_slyde}:{new_second_G_minuto} {converted_sinaw}:{new_fifth_G_minuto}")  #Functioned     10:11 AM to 8 PM

            if new_fifth_G_minuto:
                 return(f"{converted_slyde}:00 {converted_sinaw}:{new_fifth_G_minuto}")  #Functioned     10 AM to 8:50 PM

            elif new_second_G_minuto:
                  return(f"{converted_slyde}:{new_second_G_minuto} {converted_sinaw}:00") #Functioned     10:11 AM to 8 PM

            else:
                 return(f"{converted_slyde} {converted_sinaw}") #Functioned /2 need i modify mali ni
